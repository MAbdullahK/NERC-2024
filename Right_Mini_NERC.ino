...
for (int pos = 4; pos <= 50; pos += 1) { // goes from 0 degrees to 180 degrees
...
void servo_Place()
{
  for (int pos = 50; pos >= 4 ; pos -= 1) { // goes from 0 degrees to 180 degrees
    // in steps of 1 degree
    sl.write(pos);              // tell servo to go to position in variable 'pos'
    delay(5);                       // waits 15 ms for the servo to reach the position
  }
...
void reverse_Place()
{
    ...
    delay(800);
    servo_Place();
     while(digitalRead(IR_SENSOR_R) != HIGH)
    {
      apply_motors();
      delay(1);
    }
...
void jhoom()
{
  ...
  for (; k <=1; )
  {
    ...
    apply_motors();
    delay(1);
    if ((analogRead(IR_SENSOR_2) < IR_Switch || analogRead(IR_SENSOR_3) < IR_Switch || analogRead(IR_SENSOR_4) < IR_Switch) && !ghoom)
    {   
        k++;
        ghoom = true;
    }
    if (analogRead(IR_SENSOR_2) > IR_Switch && analogRead(IR_SENSOR_3) > IR_Switch && analogRead(IR_SENSOR_4) > IR_Switch) 
      ghoom = false;
  }
  sensor_Count += 2;
